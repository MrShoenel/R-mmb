...
hpsAndDatasets <- expand.grid(
  dataset = datasets_c,
  seedMult = sample.int(.Machine$integer.max, 10),
  shiftAmount = c(0, 0.1),
  doEcdf = c(TRUE, FALSE),
  ecdfMinusOne = c(TRUE, FALSE),
  retainMinValues = c(0, 1, 3, 5)
)
# Remove some cases that make no sense:
hpsAndDatasets <- hpsAndDatasets[!(hpsAndDatasets$ecdfMinusOne & !hpsAndDatasets$doEcdf), ]

#DISABLE_COMPUTATIONS <- FALSE
#temp <- get_vicinity_results(hpsAndDatasets)
```

...
```{r}
tempDsCols <- c("Dataset", "Model", "RMSE", "Rsquared", "MAE", "Resample", "Runtime")
tempDs <- do.call(rbind, lapply(datasets_r, function(d) {
  temp <- rbind(
    gridresults_r_bc[, tempDsCols],
    gridresults_r_others[, tempDsCols])
  
  return(do.call(rbind, lapply(c(caret_models_r, "bc_f", "bc_s", "bc_n"), function(m) {
    data <- temp[temp$Model == m & temp$Dataset == d, ]
...
tempDs$Model <- sapply(as.character(tempDs$Model), function(m) if (m == "bc_f") "Full" else m)
tempDs$Model <- sapply(as.character(tempDs$Model), function(m) if (m == "bc_s") "Simple" else m)
tempDs$Model <- sapply(as.character(tempDs$Model), function(m) if (m == "null") "ZeroR" else m)
...
modelsFirst <- c("ZeroR", "Simple", "Full")
modelsFirst <- c(modelsFirst, sort(setdiff(unique(as.character(tempDs$Model)), modelsFirst)))
tempDs$Model <- factor(as.character(tempDs$Model), levels = rev(modelsFirst))
```

```{r}
(ggplot(tempDs, aes(y = RMSE.mean, x = Model, fill = Model)) +
  labs(subtitle = "Comparison of RMSE against the baseline (ZeroR).") +
...
) %>% saveAndPlotAsEPS("Bayes-all-datasets_RMSE")

(ggplot(tempDs, aes(y = MAE.mean, x = Model, fill = Model)) +
  labs(subtitle = "Comparison of MAE against the baseline (ZeroR).") +
  geom_bar(position = "dodge", stat = "identity") +
  coord_flip() +
  facet_wrap(Dataset ~., scales = "free", nrow = 2) +
  theme_light() +
  theme(text = element_text(family="Consolas"),
    ...
    legend.text = element_text(size = 8),
    strip.text = element_text(color="black")) + labs(fill = "Method")
) %>% saveAndPlotAsEPS("Bayes-all-datasets_MAE")

(ggplot(tempDs, aes(y = Rsquared.mean, x = Model, fill = Model)) +
  labs(subtitle = "Comparison of R^2 against the baseline (ZeroR).") +
  ylab("R^2 = cov^2") +
  geom_bar(position = "dodge", stat = "identity") +
  coord_flip() +
  facet_wrap(Dataset ~., scales = "free", nrow = 2) +
  ...
  theme(text = element_text(family="Consolas"),
    ...
    legend.text = element_text(size = 8),
    strip.text = element_text(color="black")) + labs(fill = "Method")
) %>% saveAndPlotAsEPS("Bayes-all-datasets_RSQ")
```
